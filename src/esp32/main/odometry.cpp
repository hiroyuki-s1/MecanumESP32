...
namespace odom{
    ...
    void Odometry::update(){
        ...
        for (auto cnt : _pcnt_counter) {
            ...
            _motor_verocity_rad[i]=(m_count - prev_m_count) / _rotate_puluse_count * M_PI * 2 / _dt;
            // ESP_LOGI(common::TAG_MAIN, "dt %lf, %lf, %lf, %d", _dt, m_rad, prev_m_rad,cnt->pulse_count);
            i++;
        }
        ESP_LOGI(common::TAG_MAIN, "FRONT R % lf, FRONT L %lf, REAR R %lf, REAR L %lf",
        _motor_verocity_rad[FRONT_R], _motor_verocity_rad[FRONT_L], _motor_verocity_rad[REAR_R], _motor_verocity_rad[REAR_L]);
    }
